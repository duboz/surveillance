...
SDess::SDess(const vd::DynamicsInit& model, const vd::InitEventList& events) :
    ve::DifferentialEquation::DESS(model, events)	
{
    r = vv::toDouble(events.get("r"));
    m = vv::toDouble(events.get("m"));
...
/*
* r est la constante Beta de lequation
* -mSIR la variable interner S.
* -mSIRext les variables externes : 
*     - I
*     - les autres noeud S connecter.
* -m est la constante de mouvement.
*/
double SDess::compute(const vd::Time& /* time */) const
{
	double c = -r * mSIR[0]() * mSIRext[0]();
...
DECLARE_NAMED_DYNAMICS(sDess, model::SDess)