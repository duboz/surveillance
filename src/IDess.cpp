...
IDess::IDess(const vd::DynamicsInit& model, const vd::InitEventList& events) :
...
/*
* -mSIR la variable interner I.
* -mSIRext les variables externes : 
*      - S
*      - les autres noeud I connecter.
*/
double IDess::compute(const vd::Time& /* time */) const
{
	double c = r * mSIRext[0]() * mSIR[0]() - a * mSIR[0]();
...
DECLARE_NAMED_DYNAMICS_DBG(iDess, model::IDess)
